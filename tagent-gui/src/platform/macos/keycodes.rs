...
//
// The parser below is fully real (not a stub) so `HotkeyParser::parse`/`validate_hotkey`
// succeed identically on every OS, even though macOS's `KeyboardHook` (see `keyboard.rs`)
// does nothing with the parsed hotkey yet — only `is_key_pressed` and the hook itself are
// stubs on this platform.
