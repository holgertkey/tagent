...
// value stays alive. Creating one per call and dropping it right after `set_text` closes
// that thread before clipboard managers reliably pick up the contents, which is exactly
// what arboard's own "Clipboard was dropped very quickly after writing" warning flags.
// Keeping a single instance alive for the process lifetime avoids this, per arboard's own
// recommendation to keep `Clipboard` in more persistent state.
...
impl ClipboardManager {
    ...
    pub fn copy_selected_text(&self) -> Result<(), Box<dyn Error + Send + Sync>> {
        if std::env::var("WAYLAND_DISPLAY").is_ok() && std::env::var("DISPLAY").is_err() {
            ...
            return Err(
                "Auto-copy not supported on Wayland. Copy text manually before clicking \"Copy\"."
                    .into(),
            );
        }

        // Wait for the triggering click/keystroke to settle.
...
mod tests {
    ...
    fn test_set_text_reuses_persistent_clipboard_instance() {
        ...
        let clipboard = ClipboardManager::new();
        if clipboard
            .set_text("tagent-gui-clipboard-persistence-test")
            .is_err()
        {
        ...
        assert_eq!(
            ...
            "tagent-gui-clipboard-persistence-test"
        );
    }
}
